      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-zinc-100 to-slate-100 text-slate-800 dark:from-zinc-950 dark:via-slate-900 dark:to-zinc-950 dark:text-slate-100 flex font-sans transition-colors duration-300">
        ...
        <aside className="w-64 bg-white/85 dark:bg-zinc-900/60 backdrop-blur-xl border-r border-slate-200 dark:border-zinc-800/80 flex flex-col shrink-0 transition-colors duration-300">
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm transition-all duration-200 ${
                    isActive
                      ? "bg-indigo-50 dark:bg-indigo-500/10 border-l-2 border-indigo-650 dark:border-indigo-500 text-indigo-650 dark:text-white font-semibold"
                ...
                >
                  <Icon size={16} className={isActive ? "text-indigo-650 dark:text-indigo-400 animate-pulse" : "text-slate-400 dark:text-zinc-500"} />
                  {label}
                </button>
              );
            })}
          ...
          <div className="p-4 border-t border-slate-200 dark:border-zinc-800/80">
            {admin && (
              <div className="mb-3 bg-slate-50 dark:bg-zinc-950/40 border border-slate-200 dark:border-zinc-800/60 rounded-2xl p-3 shadow-inner">
        ...
        {activeTab === "dashboard" && (
          <div className="space-y-8">
            <div>
              <h1 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-slate-900 via-slate-800 to-slate-700 dark:from-white dark:via-zinc-200 dark:to-zinc-400 tracking-tight">Dashboard</h1>
            ...
            {loading ? (
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                {[1,2,3,4].map(i => (
                  <div key={i} className="bg-white dark:bg-zinc-900/40 border border-slate-200 dark:border-zinc-800/50 rounded-3xl p-6 animate-pulse h-32" />
                ))}
              </div>
            ) : stats && (
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                  ...
                  { label: "Verified", value: stats.verifiedCount, icon: CheckCircle, color: "from-emerald-500/10 to-teal-500/5 text-emerald-600 border-emerald-500/20 dark:from-emerald-500/20 dark:to-teal-500/10 dark:text-emerald-400 dark:border-emerald-500/20 hover:shadow-emerald-500/5", glow: "rgba(16,185,129,0.15)", lightGlow: "rgba(16,185,129,0.06)" },
                  ...
                  return (
                    <div
                      key={card.label}
                      className="bg-white dark:bg-zinc-900/40 border border-slate-200 dark:border-zinc-800/50 backdrop-blur-md rounded-3xl p-6 transition-all duration-300 hover:-translate-y-1.5 hover:border-slate-350 dark:hover:border-zinc-700/60"
                      style={{
                    ...
                    >
                      <div className="flex items-center justify-between mb-4">
                        <p className="text-xs text-slate-500 dark:text-zinc-400 font-bold uppercase tracking-wider">{card.label}</p>
            ...
            <div className="bg-white dark:bg-zinc-900/40 border border-slate-200 dark:border-zinc-800/80 rounded-3xl overflow-hidden backdrop-blur-md shadow-md dark:shadow-xl transition-colors duration-300">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-slate-50/80 dark:bg-zinc-950/40 border-b border-slate-200 dark:border-zinc-800/60 transition-colors duration-300">
                    <tr>
                      {["User Details", "Organization", "License Code", "Status", "Joined On"].map(h => (
                        <th key={h} className="text-left px-6 py-4 text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-wider">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 dark:divide-zinc-800/50 transition-colors duration-300">
                    {users.length === 0 ? (
                      <tr>
                        <td colSpan={5} className="px-6 py-12 text-center text-slate-400 dark:text-zinc-500 font-medium">No recent registrations found</td>
                      </tr>
                    ) : users.map(user => (
                      <tr key={user.id} className="hover:bg-slate-50/50 dark:hover:bg-zinc-800/20 transition-colors duration-200">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-indigo-500/10 to-purple-500/5 dark:from-indigo-500/20 dark:to-purple-500/10 border border-indigo-100 dark:border-indigo-500/25 flex items-center justify-center text-sm font-bold text-indigo-650 dark:text-indigo-300 shadow-sm">
                        ...
                        <td className="px-6 py-4">
                          <span className="text-slate-700 dark:text-zinc-300 font-semibold text-xs bg-slate-50 dark:bg-zinc-950/30 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-zinc-800/50">
                        ...
                        <td className="px-6 py-4">
                          {user.isVerified ? (
                            <span className="inline-flex items-center gap-1.5 text-xs bg-emerald-50 dark:bg-emerald-500/10 text-emerald-650 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-500/20 px-3 py-1 rounded-full font-semibold whitespace-nowrap">
                              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 dark:bg-emerald-400 animate-pulse" />
                              Verified
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1.5 text-xs bg-amber-50 dark:bg-amber-500/10 text-amber-650 dark:text-amber-400 border border-amber-100 dark:border-amber-500/20 px-3 py-1 rounded-full font-semibold whitespace-nowrap">
                              <span className="w-1.5 h-1.5 rounded-full bg-amber-500 dark:bg-amber-400 animate-pulse" />
                              Pending
                            </span>
                          )}
                        </td>
                        <td className="px-6 py-4 text-slate-400 dark:text-zinc-500 text-xs font-semibold">
        ...
        {activeTab === "smtp" && (
          <div className="space-y-8 max-w-4xl animate-fade-in">
            <div>
              <h1 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-slate-900 via-slate-800 to-slate-700 dark:from-white dark:via-zinc-200 dark:to-zinc-400 tracking-tight">SMTP Settings</h1>
            ...
            <form onSubmit={handleSmtpSave} className="bg-white dark:bg-zinc-900/40 border border-slate-200 dark:border-zinc-800/80 rounded-3xl p-8 space-y-8 backdrop-blur-md shadow-md dark:shadow-2xl relative overflow-hidden transition-colors duration-300">
              ...
              <div className="space-y-5">
                <div className="flex items-center gap-2.5 pb-2 border-b border-slate-200 dark:border-zinc-800/60">
                  <div className="w-7 h-7 rounded-lg bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center border border-indigo-100 dark:border-indigo-500/20 text-indigo-650 dark:text-indigo-400">
                    <Server size={14} />
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-wider mb-2">SMTP Username *</label>
                    <input
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-wider mb-2">SMTP Password *</label>
                    <input
                    ...
                    <label className="flex items-center gap-3 cursor-pointer group">
                      <div
                        ...
                        className={`w-12 h-6 rounded-full transition-all duration-300 flex items-center p-0.5 cursor-pointer ${
                          smtp.smtpSecure ? "bg-indigo-600 shadow-[0_0_12px_rgba(99,102,241,0.4)]" : "bg-slate-200 border border-slate-300 dark:bg-zinc-800 dark:border-zinc-700"
                        }`}
                      >
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-wider mb-2">From Email *</label>
                    <input
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-wider mb-2">From Name *</label>
                    <input
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-wider mb-2">App Name</label>
                    <input
                      value={smtp.appName || ""}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-wider mb-2">App URL</label>
                    <input
                      value={smtp.appUrl || ""}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-wider mb-2">Support Email</label>
                    <input
              ...
              <div className="space-y-5">
                <div className="flex items-center gap-2.5 pb-2 border-b border-slate-200 dark:border-zinc-800/60">
                  ...
                  <h2 className="font-bold text-slate-800 dark:text-white text-sm tracking-wide uppercase">Daily Automated Reports</h2>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-wider mb-2">Recipient Email</label>
                    <input
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-wider mb-2">Report Time (HH:MM)</label>
                    <input
                  ...
                  <div className="flex items-end pb-3">
                    <label className="flex items-center gap-3 cursor-pointer group">
                      <div
                        ...
                        className={`w-12 h-6 rounded-full transition-all duration-300 flex items-center p-0.5 cursor-pointer ${
                          smtp.cronReportEnabled ? "bg-indigo-600 shadow-[0_0_12px_rgba(99,102,241,0.4)]" : "bg-slate-200 border border-slate-300 dark:bg-zinc-800 dark:border-zinc-700"
                        }`}
                      >
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-4 border-t border-slate-200 dark:border-zinc-800/80">
                <button
                  ...
                  className="flex items-center justify-center gap-2.5 px-6 py-3.5 border border-slate-200 dark:border-zinc-800 hover:border-slate-300 dark:hover:border-zinc-700 text-slate-600 dark:text-zinc-300 bg-slate-50 dark:bg-zinc-950/20 hover:bg-slate-100 dark:hover:bg-zinc-800/40 rounded-2xl text-sm font-bold disabled:opacity-50 transition-all duration-200 active:scale-95 shadow-sm"
                >
                  {smtpTesting ? (
                    <span className="w-4 h-4 border-2 border-indigo-400 border-t-transparent rounded-full animate-spin" />
                  ) : (
                    <Send size={14} className="text-slate-400 dark:text-zinc-500" />
                  )}
                  Test Connection
                </button>
                <button
        ...
        {activeTab === "users" && (
          <div className="space-y-8 animate-fade-in">
            <div>
              ...
              <p className="text-slate-500 dark:text-zinc-400 text-sm mt-1">Full control over subscriber accounts and active organization licenses</p>
            </div>
            ...
            {stats && (
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                  { label: "Total Subscribers", value: stats.total, color: "text-slate-900 dark:text-white bg-white dark:bg-zinc-900/40 border-slate-200 dark:border-zinc-800/40" },
                  { label: "Verified Licenses", value: stats.verifiedCount, color: "text-emerald-600 dark:text-emerald-400 bg-white dark:bg-zinc-900/40 border-slate-200 dark:border-zinc-800/40 shadow-[0_4px_20px_rgba(16,185,129,0.03)] dark:shadow-[0_4px_20px_rgba(16,185,129,0.05)]" },
                  { label: "Unverified Requests", value: stats.unverifiedCount, color: "text-amber-605 dark:text-amber-400 bg-white dark:bg-zinc-900/40 border-slate-200 dark:border-zinc-800/40" },
                  { label: "Registered This Week", value: stats.weekCount, color: "text-indigo-655 dark:text-indigo-400 bg-white dark:bg-zinc-900/40 border-slate-200 dark:border-zinc-800/40" },
                ].map(s => (
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white/80 dark:bg-zinc-900/20 border border-slate-200 dark:border-zinc-800/50 p-4 rounded-3xl backdrop-blur-md shadow-sm">
              <div className="relative w-full md:max-w-sm">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-zinc-950/60 border border-slate-200 dark:border-zinc-800/80 rounded-2xl text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent placeholder-slate-400 dark:placeholder-zinc-500 transition-all shadow-inner"
                />
              </div>
              <div className="flex gap-1 bg-slate-100 dark:bg-zinc-950/80 border border-slate-200 dark:border-zinc-800/60 rounded-2xl p-1 w-full md:w-auto overflow-x-auto shrink-0">
                  ...
                  <thead className="bg-slate-50 dark:bg-zinc-950/40 border-b border-slate-200 dark:border-zinc-800/60">
                    <tr>
                      {["User Details", "Organization", "Active Plan", "License Status", "Subscription End Date", "Actions"].map(h => (
                        <th key={h} className="text-left px-6 py-4.5 text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-wider">
                          {h}
                        </th>
                      ))}
                    </tr>
                      ...
                      return (
                        <tr key={user.id} className="hover:bg-slate-50/50 dark:hover:bg-zinc-800/20 transition-colors duration-200">
                          ...
                          <td className="px-6 py-4.5 whitespace-nowrap">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <span className="text-slate-500 dark:text-zinc-550 text-xs">{user.email}</span>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="px-6 py-4.5 whitespace-nowrap">
                            {org ? (
                              <div>
                                <p className="text-slate-700 dark:text-zinc-200 text-xs font-semibold bg-slate-50 dark:bg-zinc-950/20 border border-slate-200 dark:border-zinc-800/40 rounded-lg px-2.5 py-1 inline-block">{org.name}</p>
                          ...
                          <td className="px-6 py-4.5 whitespace-nowrap">
                            {org ? (
                              org.status === "ACTIVE" ? (
                                <span className="inline-flex items-center gap-1.5 text-xs bg-emerald-50 dark:bg-emerald-500/10 text-emerald-650 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-500/20 px-3 py-1 rounded-full font-semibold">
                                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 dark:bg-emerald-400 animate-pulse" />
                                  Active
                                </span>
                              ) : (
                                <span className="inline-flex items-center gap-1.5 text-xs bg-rose-50 dark:bg-rose-500/10 text-rose-650 dark:text-rose-400 border border-rose-100 dark:border-rose-500/20 px-3 py-1 rounded-full font-semibold">
                          ...
                          <td className="px-6 py-4.5 whitespace-nowrap">
                            {org ? (
                              org.subscriptionEndDate ? (
                                isExpired ? (
                                  <div>
                                    <p className="text-rose-600 dark:text-rose-400 text-xs font-bold flex items-center gap-1">
                              ...
                              <div className="flex items-center gap-2 text-xs">
                                ...
                                {org.planType === "FREE_TRIAL" ? (
                                  <button
                                    ...
                                    disabled={submittingOrgId === org.id}
                                    className="px-2.5 py-1.5 bg-amber-50 dark:bg-amber-500/5 hover:bg-amber-100 dark:hover:bg-amber-500/10 border border-amber-200 dark:border-amber-500/25 hover:border-amber-450 dark:hover:border-amber-500/40 text-amber-705 dark:text-amber-400 rounded-xl transition-all duration-200 font-extrabold disabled:opacity-50 hover:scale-105"
                                  >
                                    Activate
                                  </button>
                                ) : (
                                  <button
                                    ...
                                    disabled={submittingOrgId === org.id}
                                    className="px-2.5 py-1.5 bg-slate-100 dark:bg-zinc-800/30 hover:bg-slate-200 dark:hover:bg-zinc-800/60 border border-slate-200 dark:border-zinc-700/40 text-slate-655 dark:text-zinc-300 rounded-xl transition-all duration-200 font-semibold disabled:opacity-50 hover:scale-105"
                                  >
                                    Downgrade
                                  </button>
                                )}
        ...
        {activeTab === "cron" && (
          <div className="space-y-8 max-w-4xl animate-fade-in">
            <div>
              <h1 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-slate-900 via-slate-800 to-slate-700 dark:from-white dark:via-zinc-200 dark:to-zinc-400 tracking-tight">Cron Jobs</h1>
              <p className="text-slate-500 dark:text-zinc-400 text-sm mt-1">Automated background tasks. Trigger manually for testing or run logs generation.</p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6">
              {cronJobs.map(job => (
                <div key={job.action} className="group relative bg-white dark:bg-zinc-900/40 border border-slate-200 dark:border-zinc-800/80 rounded-3xl p-6 backdrop-blur-md transition-all duration-300 hover:border-slate-350 dark:hover:border-zinc-700/60 shadow-md dark:shadow-lg">
          ...
          <div className="relative bg-white dark:bg-zinc-900/80 border border-slate-200 dark:border-zinc-800/80 rounded-3xl p-8 max-w-md w-full space-y-6 shadow-2xl overflow-hidden">
            ...
            <div>
              ...
              {extendModalOrg.endDate && (
                <p className="text-xs text-slate-550 dark:text-zinc-550 mt-2 flex items-center gap-1.5 font-medium">
                  <span className="w-1.5 h-1.5 rounded-full bg-slate-400 dark:bg-zinc-650" />
                  Current Expiration Date: <span className="text-slate-700 dark:text-zinc-300 font-bold">{new Date(extendModalOrg.endDate).toLocaleDateString("en-IN")}</span>
                </p>
              )}
            </div>
              ...
              <div className="grid grid-cols-3 gap-2.5">
                  ...
                  <button
                    ...
                    className={`py-2.5 px-1.5 text-xs rounded-xl font-bold border transition-all duration-200 hover:scale-105 active:scale-95 ${
                      customDays === d
                        ...
                        : "bg-slate-50 dark:bg-zinc-950/40 border-slate-200 dark:border-zinc-800/80 text-slate-550 dark:text-zinc-450 hover:text-slate-800 dark:hover:text-zinc-100 hover:border-slate-350 dark:hover:border-zinc-700"
                    }`}
                  >
                    +{d} Days
                  </button>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="Custom"
                    className="w-full px-2.5 py-2.5 bg-slate-50 dark:bg-zinc-950/60 border border-slate-200 dark:border-zinc-800/80 rounded-xl text-slate-900 dark:text-white text-center text-xs font-bold focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent placeholder-slate-400 dark:placeholder-zinc-650"
                  />
                </div>
              </div>
            </div>
